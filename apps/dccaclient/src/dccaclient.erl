        ...
        {ok, _} ->
            ...
            rate_service(gprs,
                         {update,
                          ...
                          SessionId,
                          ReqN,
                          {ServiceId, RatingGroup, 0, VolumeBytes},
                          TimeToConsumeBytes}),
    ...
    CCR = #'CCR'{'Session-Id' = SessionId,
                 ...
                 'Event-Timestamp' =
                     [calendar:now_to_local_time(
                          erlang:timestamp())],
                 'Subscription-Id' =
                     [#'Subscription-Id'{'Subscription-Id-Type' = ?'MSISDN',
                                         'Subscription-Id-Data' = MSISDN},
                      #'Subscription-Id'{'Subscription-Id-Type' = ?'IMSI',
...
rate_service(gprs,
             {update,
              ...
              SessionId,
              ReqN,
              {ServiceId, RatingGroup, ConsumedBytes, RemainingBytes},
              TimeToConsumeBytes}) ->
    ReqN2 = ReqN + 1,
                ...
                NewRemainingBytes when NewRemainingBytes > 0 ->
                    rate_service(gprs,
                                 {update,
                                  ...
                                  TimeToConsumeBytes});
                NewRemainingBytes when NewRemainingBytes =< 0 ->
                    lager:info("Last request: ~w | ~w | ~w ~n",
                    ...
                    rate_service(gprs,
                                 {terminate,
                                  ...
                                  SessionId,
                                  ReqN2,
                                  {ServiceId, RatingGroup, RemainingBytes, 0},
                                  TimeToConsumeBytes})
            end;
        {error, Err} ->
            lager:error("Error: ~w~n", [Err])
...
generate_MSCC(ServiceId, RatingGroup, ConsumedBytes, RemainingBytes) ->
    if (ConsumedBytes == 0) and (RemainingBytes > 0) ->
           %% First request. Must send RSU and no USU.
           MSCC =
               #'CCR'{'Multiple-Services-Credit-Control' =
                          [#'Multiple-Services-Credit-Control'{'Requested-Service-Unit' =
       ...
       (ConsumedBytes /= 0) and (RemainingBytes =< 0) ->
           ...
           MSCC =
               #'CCR'{'Multiple-Services-Credit-Control' =
                          [#'Multiple-Services-Credit-Control'{'Used-Service-Unit' =
                                                                   [#'Used-Service-Unit'{'CC-Total-Octets'
                                                                                             =
                                                                                             [ConsumedBytes]}],
                                                               'Service-Identifier' = [ServiceId],
